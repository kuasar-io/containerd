...
type CreateTaskRequest struct {
	...
	// TaskAddress is the task api address, if it is non-empty,
	// it means that there is already a running task api server(maybe provided by sandbox),
	// just create a client to call the Task API directly, without shim process management.
	TaskAddress string `protobuf:"bytes,11,opt,name=task_address,json=taskAddress,proto3" json:"task_address,omitempty"`
	// Bundle is the bundle belongs to this task,
	// as we may call sandbox api to prepare the task before task api is called,
	// the bundle may be prepared by the sandbox, if so, containerd should skip
	// the bundle creation before call the Task Create API
...
func (x *CreateTaskRequest) GetTaskAddress() string {
	if x != nil {
...
var file_github_com_containerd_containerd_api_services_tasks_v1_tasks_proto_rawDesc = []byte{
	...
	0x2e, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x22, 0x95, 0x03, 0x0a, 0x11, 0x43, 0x72, 0x65, 0x61, 0x74,
	...
	0x61, 0x74, 0x68, 0x12, 0x21, 0x0a, 0x0c, 0x74, 0x61, 0x73, 0x6b, 0x5f, 0x61, 0x64, 0x64, 0x72,
	0x65, 0x73, 0x73, 0x18, 0x0b, 0x20, 0x01, 0x28, 0x09, 0x52, 0x0b, 0x74, 0x61, 0x73, 0x6b, 0x41,
	0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x12, 0x16, 0x0a, 0x06, 0x62, 0x75, 0x6e, 0x64, 0x6c, 0x65,
	0x18, 0x0c, 0x20, 0x01, 0x28, 0x09, 0x52, 0x06, 0x62, 0x75, 0x6e, 0x64, 0x6c, 0x65, 0x22, 0x49,
